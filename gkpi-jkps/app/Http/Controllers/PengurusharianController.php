...
use App\Kas;
use App\KasKeuangan;
use App\Renungan_Pelayan;
use App\Program_Pelayan;
use App\Berita_Pelayan;
use App\Pelayan_PersembahanIbadah;
use App\Pelayan_PersembahanKhusus;
use App\pengeluaranpersembahankhusus;
...
class PengurusharianController extends Controller
{
    ...
    function index(Request $request)
    {
        ...
        return view("Pengurusharian.index", [
    ...
    function detailstatistik($name)
    {
        if ($name == "Jumlah+Keluarga") {
            ...
            $jemaat = Jemaat::where('status_gereja', '!=', 'Aktif')->get();
            return view('Pengurusharian.datajemaattidakaktif', [
                "jemaats" => $jemaat
            ]);
    ...
    function datakeluargatidakaktif(Request $request)
    {
        // Change this pagination if you want to increase pagination
        $keluarga = Keluarga::with('sektor')->where('status', '!=', 'Aktif')->get();
        return view('Pengurusharian.datakeluargatidakaktif', [
            "keluargas" => $keluarga
        ]);
    }
    ...
    {
        $sektors = Sektor::get();
        return view("Pengurusharian.formkeluarga", ['sektors' => $sektors]);
    }
    function formkeluargaprocess(Request $request)
    {
        ...
        return redirect()->route('pengurusharian.datakeluarga')->with('success', 'Data Keluarga Berhasil Disimpan!');
    }
    ...
    ', [$id]);
        ...
        return view('Pengurusharian.detailkeluarga', ["keluarga" => $keluarga, 'lampiran' => $lampiran,  'jemaats' => $jemaat]);
    }
    ...
    public function keluargabaru($no_kk, $id)
    {
        ...
        return view('Pengurusharian.formkeluargabaru', compact('keluarga', 'no_kk', 'jemaat'));
    }
    public function storekeluargabaru(Request $request, $no_kk, $id)
    {
        ...
        return redirect("pengurusharian/data/keluarga/$request->no_kk")->with('success', 'Data Keluarga Berhasil Disimpan!');
    }
    public function editdatakeluarga(Request $request, $id)
    {
        ...
        $sektors = Sektor::get();
        $lampiran = explode("#", $keluarga['lampiran']);
        return view('Pengurusharian.editdatakeluarga', ["keluarga" => $keluarga, 'lampiran' => $lampiran, 'sektors' => $sektors]);
    }
    public function update(Request $request, $id)
    {
        ...
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.datakeluarga')->with('warning', 'Tidak ada perubahan pada data Keluarga!');
    ...
    function datajemaatulangtahun()
    {
        ...
        return view('Pengurusharian.ulangtahun', [
    ...
    //         return redirect('/pengurusharian/data/keluarga/' . $idKeluarga);
    //     }
    // }
    ...
    public function formjemaatprocess(Request $request, $idKeluarga)
    {
        ...
        if ($request->hasFile("profile")) {
            ...
            // Jika tidak ada file yang diunggah, tambahkan default profile ke dalam array
            $arrNameProfile[] = '/profile/jemaat/default.png';
        }
        ...
        // Validasi unik untuk nik, nomor surat baptis, dan nomor surat sidi jika data ada
        if (Jemaat::where('nik', $request->nik)->exists()) {
            return redirect()->back()->with('error', 'NIK sudah terdaftar!');
        }

        if ($request->baptis == 'Ya' && Baptis::where('no_surat_baptis', $request->nomor_surat_baptis)->exists()) {
            return redirect()->back()->with('error', 'Nomor surat baptis sudah terdaftar!');
        }

        if ($request->sidi == 'Ya' && Sidi::where('no_surat_sidi', $request->nomor_surat_sidi)->exists()) {
            return redirect()->back()->with('error', 'Nomor surat sidi sudah terdaftar!');
        }
        ...
        $jemaat->nik = $request->nik;
        $jemaat->name = $request->name;
        $jemaat->username = NULL;
        $jemaat->jenis_kelamin = $request->jenis_kelamin;
        $jemaat->password = NULL;
        $jemaat->alamat = $request->alamat;
        $jemaat->tempat_lahir = $request->tempat_lahir;
        $jemaat->tanggal_lahir = $request->tanggal_lahir;
        $jemaat->status_gereja = $request->status;
        ...
        if (!$jemaat->save()) {
            ...
            // Hanya tambahkan data baptis jika request->baptis == 'Ya'
            if ($request->baptis == 'Ya') {
                ...
                if (!$baptis->save()) {
                    Jemaat::where("nik", $request->nik)->delete();
                    unlink(public_path() . $profile);
                    foreach (explode("#", $fileName) as $l) {
                        unlink(public_path() . $l);
                    }
                    return redirect()->back()->with('error', 'Gagal menyimpan data baptis.');
                }
            }

            // Hanya tambahkan data sidi jika request->sidi == 'Ya'
            if ($request->sidi == 'Ya') {
                $sidi = new Sidi();
                $sidi->tgl_sidi = $request->tanggal_sidi;
                $sidi->nama_pendeta_sidi = $request->nama_pendeta_sidi;
                ...
                if (!$sidi->save()) {
                    ...
                    return redirect()->back()->with('error', 'Gagal menyimpan data sidi.');
                }
            }
            ...
            if (!$jemaat_keluarga->save()) {
                ...
                return redirect()->back()->with('error', 'Gagal menyimpan data keluarga jemaat.');
            }

            return redirect('/pengurusharian/data/keluarga/' . $idKeluarga)->with('success', 'Data Jemaat Berhasil Disimpan!');
        }
    }
    ...
    function datastatistik()
    {
        ...
        return view('Pengurusharian.datastatistik', [
    ...
    //     return view('Pengurusharian.detailjemaat', ['jemaat'=> $jemaat,'lampiran'=> $lampiran, 'baptis'=>$baptis, 'sidi'=>$sidi, 'file_surat_baptis'=>$file_surat_baptis, 'file_surat_sidi'=>$file_surat_sidi]);
    // }
    ...
    function detailjemaat($nik)
    {
        ...
        return view('Pengurusharian.detailjemaat', [
            'jemaat' => $jemaat,
            'lampiran' => $lampiran,
            'baptis' => $baptis,
            'sidi' => $sidi,
    ...
    function editdetailjemaat($nik)
    {
        ...
        $file_surat_baptis = $baptis ? explode("#", $baptis['file_surat_baptis']) : [];
        $file_surat_sidi = $sidi ? explode("#", $sidi['file_surat_sidi']) : [];
        return view('Pengurusharian.editdatajemaat', ['jemaat' => $jemaat, 'lampiran' => $lampiran, 'baptis' => $baptis, 'sidi' => $sidi, 'file_surat_baptis' => $file_surat_baptis, 'file_surat_sidi' => $file_surat_sidi, 'sektors' => $sektors]);
    }
    ...
    function editdetailjemaat2($idKeluarga, $nik)
    {
        ...
        $file_surat_baptis = $baptis ? explode("#", $baptis['file_surat_baptis']) : [];
        $file_surat_sidi = $sidi ? explode("#", $sidi['file_surat_sidi']) : [];
        return view('Pengurusharian.editdatajemaat', ['jemaat' => $jemaat, 'lampiran' => $lampiran, 'baptis' => $baptis, 'sidi' => $sidi, 'file_surat_baptis' => $file_surat_baptis, 'file_surat_sidi' => $file_surat_sidi, 'sektors' => $sektors, 'idKeluarga' => $idKeluarga]);
    }
    ...
    function updateJemaat(Request $request, $nik)
    {
        ...
        if (
            array_diff_assoc($newJemaatData, $jemaat->toArray()) == [] &&
            ($newBaptisData == null || array_diff_assoc($newBaptisData, $baptis->toArray() ?? []) == []) &&
            ($newSidiData == null || array_diff_assoc($newSidiData, $sidi->toArray() ?? []) == [])
        ) {
            return redirect()->back()->with('warning', 'Tidak ada perubahan pada data Jemaat, Baptis, atau Sidi!');
        }
    ...
    function updateJemaat2(Request $request, $nik)
    {
        $jemaat = Jemaat::where("nik", $nik)->first();
        $baptis = Baptis::where("nik", $nik)->first();
        $sidi = Sidi::where("nik", $nik)->first();
    ...
    public function showsektoranggota()
    {
        ...
        if (request()->has('sektor_id')) {
            ...
            // Mengambil nik dari jemaat yang berhubungan dengan keluarga dan peran sebagai Penatua atau Calon Penatua
            $niks = Jemaat::whereHas('keluargaJemaat', function ($query) use ($keluargas) {
                $query->whereIn('no_kk', $keluargas)->where('status_anggota', 'Aktif');
            })->whereHas('pelayanGereja', function ($query) use ($today) {
                $query->whereIn('peran', ['Penatua Aktif', 'Calon Penatua Aktif'])
                    ->where(function ($query) use ($today) {
                        $query->whereNull('tanggal_akhir_jabatan')
                            ->orWhere('tanggal_akhir_jabatan', '>', $today);
                    });
            })->pluck('nik');
        ...
        return view('Pengurusharian.sektorshowanggota', compact('sektors', 'jemaats', 'keluarga'));
    }
    ...
    public function storesektor(Request $request)
    {
        ...
        return redirect()->route('pengurusharian.sektor')->with('success', 'Sektor berhasil ditambahkan!');
    }

    public function showsektor()
    {
        $sektors = Sektor::get();
        return view('Pengurusharian.sektorshow', compact('sektors'));
    }
    ...
    function updatesektor(Request $request)
    {
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.Sektor')->with('warning', 'Tidak ada perubahan pada data Sektor!');
        }

        // Update data sektor di database
        DB::table('sektor')->where('id', $id)->update($newData);

        return redirect()->route('pengurusharian.Sektor')->with('success', 'Data Sektor Sudah Berhasil Diubah');
    }
    ...
    function pelayan()
    {
        ...
        return view('Pengurusharian.datapelayan', [
            "pelayanas" => $pelayan
        ]);
    }
    function formpelayan(Request $request)
    {
        ...
        return view('Pengurusharian.formdatapelayan', compact('jemaat'));
    }
    function formpelayanprocess(Request $request)
    {
        ...
        // Daftar peran yang tidak memerlukan validasi tambahan
        $excludedRoles = ['Penasehat Jemaat', 'Pengawas Harta Benda', 'Penatua Aktif', 'Calon Penatua Aktif'];

        // Jika peran tidak ada dalam daftar excludedRoles, maka lakukan validasi tambahan
        if (!in_array($request->peran, $excludedRoles)) {
            $existingRole = PelayanGereja::where('peran', $request->peran)
                ->where(function ($query) {
                    $query->whereNull('tanggal_akhir_jabatan')
                        ->orWhere('tanggal_akhir_jabatan', '');
                })
                ->first();
            if ($existingRole) {
                return redirect()->back()->with('error', 'Peran tersebut sudah ada dan masih aktif, tidak boleh ditambahkan lagi!');
            }
        }
        ...
        return redirect('/pengurusharian/pelayangereja')->with('success', 'Data Pelayan Berhasil Disimpan!');
    }
    ...
    public function editdatapelayan($id)
    {
        ...
        return view('Pengurusharian.editpelayan', [
            "pelayanas" => $pelayan
        ], compact('jemaat'));
    }


    //     function updatedatapelayan(Request $request,$id_user, $id)
    // {
    ...
    //         'tanggal_akhir_jabatan' => $request->tanggal_akhir_jabatan ?: null, // Set NULL jika kosong
    //     ];
    ...
    //     // Bandingkan data baru dengan data yang ada
    //     if ($newData == $currentData) {
    //         return redirect('pengurusharian/pelayangereja')->with('warning', 'Tidak ada perubahan pada data Pelayan Gereja!');
    //     }
    ...
    //     // Jika peran tidak ada dalam daftar excludedRoles, maka lakukan validasi tambahan
    //     if (!in_array($request->peran, $excludedRoles)) {
    //         $existingRole = DB::table('pelayan_gereja')->where('peran', $request->peran)
    ...
    //     return redirect('pengurusharian/pelayangereja')->with('success', 'Data Pelayan Sudah Berhasil Diubah');
    // }
    ...
    function updatedatapelayan(Request $request, $id_user, $id)
    {
        ...
        // Jika $id dan $id_user sama, maka validasi tanggal akhir jabatan
        if ($id == $id_user) {
            $today = Carbon::today()->format('Y-m-d');
            if ($request->tanggal_akhir_jabatan <= $today) {
                return redirect()->back()->with('error', 'Pelayan masih aktif, Tanggal akhir jabatan harus lebih besar dari hari ini!');
            }
        }
    ...
    public function showrenungan()
    {
        ...
        return view('Pengurusharian.renunganshow', compact('jadwal_ibadah', 'renungan'));
    }
    public function detailrenungan($id)
    {
        $renungan = Renungan::where("id", $id)->first();
        return view('Pengurusharian.detailrenungan', compact('renungan'));
    }
    public function createrenungan()
    {
        return view('Pengurusharian.createrenungan');
    }
    public function storerenungan(Request $request, $id_user)
    {
        // Validasi input
        $data = $request->validate([
        ...
        // Simpan data ke tabel renungan_pelayan
        Renungan_Pelayan::create([
            'id_renungan' => $renungan->id,
            'id_pelayan' => $id_pelayan,
        ]);

        return redirect()->route('pengurusharian.renunganshow')->with('success', 'Renungan berhasil ditambahkan!');
    }

    public function editrenungan($id_user, $id)
    {
        $renungan = DB::table('renungan')
            ->where('id', $id)
            ->get();
        return view('Pengurusharian.editrenungan', ['renungan' => $renungan]);
    }
    public function updaterenungan(Request $request, $id_user)
    {
        $id = $request->id;
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.renunganshow')->with('warning', 'Tidak ada perubahan pada data Renungan!');
        }
        ...
        $renunganPelayan = Renungan_Pelayan::where('id_renungan', $id)->where('id_pelayan', $id_pelayan)->first();

        if ($renunganPelayan) {
            // Jika sudah ada, update waktu updated_at secara langsung
            Renungan_Pelayan::where('id_renungan', $id)
        ...
        return redirect()->route('pengurusharian.renunganshow')->with('success', 'Renungan Sudah Berhasil Diubah');
    }
    ...
    public function showjadwal()
    {
        ...
        return view('Pengurusharian.jadwal', compact('jadwal_ibadah', 'jadwal_pelayanan'));
    }

    public function createjadwal()
    {
        return view('Pengurusharian.createjadwal');
    }
    public function storejadwal(Request $request)
    {
        ...
        return redirect()->route('pengurusharian.jadwal')->with('success', 'Jadwal ibadah berhasil ditambahkan!');
    }
    ...
    public function editjadwal($id)
    {
        ...
        return view('Pengurusharian.editjadwal', ['jadwal_ibadah' => $jadwal_ibadah, 'lampiran' => $lampiran]);
    }
    public function updatejadwal(Request $request)
    {
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.jadwal')->with('warning', 'Tidak ada perubahan pada data Jadwal Ibadah!');
        }
        ...
        return redirect()->route('pengurusharian.jadwal')->with('success', 'Jadwal Ibadah Sudah Berhasil Diubah');
    }
    ...
    //     return view('Pengurusharian.pelayanan', compact('pelayan', 'pelayans', 'jadwalPelayanan', 'jadwalIbadah'));
    // }
    public function showpelayanan(Request $request)
    {
        // Dapatkan tanggal hari ini menggunakan Carbon
        $today = Carbon::today()->toDateString();
        ...
        return view('Pengurusharian.pelayanan', compact('pelayan', 'pelayans', 'jadwalPelayanan', 'jadwalIbadah', 'jumlahPelayanan'));
    }
    ...
    public function createpelayanan()
    {
        ...
        // Query untuk mendapatkan data pelayan gereja dengan kondisi tertentu
        $pelayan_gereja = PelayanGereja::with('jemaat')
            ->whereNull('tanggal_akhir_jabatan')
            ->orWhere('tanggal_akhir_jabatan', '>', $today)
            ->get();
        return view('Pengurusharian.createpelayanan', compact('pelayan_gereja'));
    }
    ...
    public function storepelayanan(Request $request, $id_jadwal_ibadah)
    {
        ...
        if (!empty($dataToInsert)) {
            ...
            return redirect()->route('pengurusharian.jadwal')->with('success', 'Jadwal pelayanan berhasil ditambahkan!');
        } else {
    ...
    function detailpelayanan(Request $request, $id_jadwal_ibadah)
    {
        ...
        return view('Pengurusharian.detailpelayanan', ['status_pelayanan' => $status_pelayanan, 'persembahan' => $persembahan, 'penerima_tamu' => $penerima_tamu, 'results' => $results, 'results_persembahan' => $results_persembahan, 'results_penerima_tamu' => $results_penerima_tamu]);
    }
    ...
    public function editpelayanan($id_jadwal_ibadah)
    {
        ...
        return view('Pengurusharian.editpelayanan', [
    ...
    public function updatepelayanan(Request $request, $id_jadwal_ibadah)
    {
        ...
        return redirect("pengurusharian/jadwal/pelayanan/$id_jadwal_ibadah")->with('success', 'Data pelayanan berhasil diperbarui.');
    }
    ...
    //     return redirect()->route('pengurusharian.detailpelayanan', ['id' => $id])->with('success', 'Jadwal Pelayanan Sudah Berhasil Diubah');
    // }
    ...
    function detailtataibadah()
    {
        ...
        $renungan = Renungan::latest()->take(10)->get();
        return view('Pengurusharian.detailibadah', compact('jadwal_ibadah', 'tata_ibadah', 'renungan'));
    }
    public function createtata()
    {

        $jadwal_ibadahs = jadwal_ibadah::whereNull('tata_ibadah')->latest()->get();
        return view('Pengurusharian.createtata', compact('jadwal_ibadahs'));
    }
    function formibadah(Request $request)
    {

        return view("Pengurusharian.tambahibadah");
    }
    ...
    function tatastore(Request $request)
    {
        ...
        return redirect()->route('pengurusharian.detailibadah')->with('success', 'Tata Ibadah Sudah Berhasil Ditambahkan');
    }
    public function edittataibadah($id)
    {
        $tata_ibadah = DB::table('tata_ibadah')
            ->where('id', $id)
            ->get();
        return view('Pengurusharian.edittataibadah', ['tata_ibadah' => $tata_ibadah]);
    }
    function updatetataibadah(Request $request)
    {
        ...
        return redirect()->route('pengurusharian.detailibadah')->with('success', 'Tata Ibadah Sudah Berhasil Diubah');
    }
    //PROGRAM
    ...
    public function showprogramkerja()
    {
        ...
        return view('Pengurusharian.programkerja', compact('program'));
    }
    public function showRAPB()
    {
        $program = Program::where('jenis_program', 'Rancangan Anggaran Penerimaan dan Belanja')->latest()->get();
        return view('Pengurusharian.RAPB', compact('program'));
    }
    public function createprogram()
    {
        return view('Pengurusharian.createprogram');
    }
    public function storeprogram(Request $request, $id_user)
    {
        ...
        if ($request->jenis_program == 'Rancangan Program Kerja') {
            return redirect()->route('pengurusharian.showprogramKerja')->with('success', 'Program Kerja berhasil ditambahkan!');
        } elseif ($request->jenis_program == 'Rancangan Anggaran Penerimaan dan Belanja') {
            return redirect()->route('pengurusharian.showRAPB')->with('success', 'Rancangan Anggaran Penerimaan dan Belanja berhasil ditambahkan!');
        }
    }
    ...
    public function editprogram($id_user, $id)
    {
        $program = Program::where('id', $id)->first();

        $lampiran = explode("#", $program['lampiran']);
        return view('Pengurusharian.editprogram', ['program' => $program, 'lampiran' => $lampiran]);
    }
    public function updateprogram(Request $request, $id_user, $id)
    {
        $program = Program::where('id', $id)->first();
        $arrName = [];
        ...
        if ($newData == $currentData) {
            if ($program->jenis_program == 'Rancangan Program Kerja') {
                return redirect()->route('pengurusharian.showprogramKerja')->with('warning', 'Tidak ada perubahan pada data Program!');
            } elseif ($program->jenis_program == 'Rancangan Anggaran Penerimaan dan Belanja') {
                return redirect()->route('pengurusharian.showRAPB')->with('warning', 'Tidak ada perubahan pada data Program!');
            }
        }
        ...
        $currentTimestamp = Carbon::now();
        // Cek apakah entry sudah ada di renungan_pelayan 
        $programPelayan = Program_Pelayan::where('id_program', $id)->where('id_pelayan', $id_pelayan)->first();

        if ($programPelayan) {
            // Jika sudah ada, update waktu updated_at secara langsung
            Program_Pelayan::where('id_program', $id)
                ->where('id_pelayan', $id_pelayan)
                ->update([
                    'updated_at' => $currentTimestamp
                    // tambahkan kolom-kolom lain yang ingin diupdate beserta nilainya di sini
        ...
        // Penentuan arah redirect berdasarkan jenis_program yang baru saja dimasukkan
        if ($request->jenis_program == 'Rancangan Program Kerja') {
            return redirect()->route('pengurusharian.showprogramKerja')->with('success', 'Program Kerja berhasil diubah!');
        } elseif ($request->jenis_program == 'Rancangan Anggaran Penerimaan dan Belanja') {
            return redirect()->route('pengurusharian.showRAPB')->with('success', 'Rancangan Anggaran Penerimaan dan Belanja berhasil diubah!');
        }
    }
    ...
    public function showberita()
    {
        ...
        return view('Pengurusharian.databerita', compact('berita_gereja'));
    }
    ...
    public function detailberita($id)
    {
        ...
        return view('Pengurusharian.detailberita', compact('berita_gereja'));
    }
    public function createberita()
    {
        return view('Pengurusharian.createberita');
    }

    public function storeberita(Request $request, $id_user)
    {
        ...
        $berita = BeritaGereja::create($data);
        ...
        // Simpan data ke tabel program_pelayan
        Berita_Pelayan::create([
            'id_berita' => $berita->id,
            'id_pelayan' => $id_pelayan,
        ]);

        // Redirect kembali dengan pesan sukses
        return redirect()->route('pengurusharian.berita')->with('success', 'Berita berhasil ditambahkan');
    }


    public function editberita($id_user, $id)
    {
        $berita_gereja = DB::table('berita_gereja')
            ->where('id', $id)
            ->get();
        return view('Pengurusharian.editberita', ['berita_gereja' => $berita_gereja]);
    }

    public function updateberita(Request $request, $id_user, $id)
    {
        ...
        if ($newData == $currentData && !$request->hasFile('lampiran')) {
            return redirect()->route('pengurusharian.berita')->with('warning', 'Tidak ada perubahan pada data Berita!');
        }
        ...
        $currentTimestamp = Carbon::now();
        // Cek apakah entry sudah ada di renungan_pelayan 
        $beritaPelayan = Berita_Pelayan::where('id_berita', $id)->where('id_pelayan', $id_pelayan)->first();

        if ($beritaPelayan) {
            // Jika sudah ada, update waktu updated_at secara langsung
            Berita_Pelayan::where('id_berita', $id)
                ->where('id_pelayan', $id_pelayan)
                ->update([
                    'updated_at' => $currentTimestamp
                    // tambahkan kolom-kolom lain yang ingin diupdate beserta nilainya di sini
        ...
        // Redirect kembali dengan pesan sukses
        return redirect()->route('pengurusharian.berita')->with('success', 'Berita berhasil diubah');
    }
    ...
    public function pemasukanpersembahanibadah()
    {
        ...
        return view('Pengurusharian.dataPemasukanPersembahanIbadah', compact('pemasukan'));
    }
    ...
    public function pengeluaranpersembahanibadah()
    {
        ...
        return view('Pengurusharian.dataPengeluaranPersembahanIbadah', compact('pengeluaran'));
    }
    public function pembagianpersembahanibadah()
    {
        $kas = Kas::all();
        return view('Pengurusharian.dataPembagianPersembahanIbadah', compact('kas'));
    }
    public function detailpembagianpersembahanibadah($id)
    {
        ...
        return view('Pengurusharian.detailpembagianpersembahanibadah', compact('persembahan', 'kasList'));
    }
    ...
    public function pemasukanpersembahandiakonia()
    {
        ...
        return view('Pengurusharian.dataPemasukanPersembahanDiakonia', compact('pemasukan'));
    }
    public function pengeluaranpersembahandiakonia()
    {
        ...
        return view('Pengurusharian.dataPengeluaranPersembahanDiakonia', compact('pengeluaran'));
    }

    public function pemasukanpersembahankhusus()
    {
        $datakeuangan = PersembahanKhusus::all()->where('jenis_keuangan', 'Pemasukan');
        return view('Pengurusharian.dataPemasukanpersembahankhusus', compact('datakeuangan'));
    }
    public function pengeluaranpersembahankhusus()
    {
        $datakeuangan = PersembahanKhusus::all()->where('jenis_keuangan', 'Pengeluaran');
        return view('Pengurusharian.dataPengeluaranpersembahankhusus', compact('datakeuangan'));
    }
    ...
    public function pengeluarankas()
    {
        $pengeluaran = Keuangan::where('kategori', 'Kas')
            ...
            ->get();
        return view('Pengurusharian.dataPengeluaranKas', compact('pengeluaran'));
    }
    ...
    function formkeuangan(Request $request)
    {
        ...
        return view('Pengurusharian.formtambahkeuangan', compact('keluarga', 'kas'));
    }
    ...
    function formkeuanganprocess(Request $request, $id_user)
    {
        if ($request->kategori == 'Persembahan' && $request->jenis_keuangan == 'Pemasukan') {
            ...
            return redirect()->route('pengurusharian.dataPemasukanPersembahanIbadah');
            ...
            return redirect()->route('pengurusharian.pengeluarankas');
            ...
            if ($request->jenis_keuangan == 'Pemasukan') {
                ...
                return redirect()->route('pengurusharian.dataPengeluaranPersembahanIbadah');
            }
            ...
            if ($request->jenis_keuangan == 'Pemasukan') {
                return redirect()->route('pengurusharian.datapemasukanpersembahanKhusus');
            } else {
                return redirect()->route('pengurusharian.datapengeluaranpersembahanKhusus');
            }
            ...
            if ($request->jenis_keuangan == 'Pemasukan') {
                return redirect()->route('pengurusharian.dataPemasukanPersembahanDiakonia');
            } else {
                return redirect()->route('pengurusharian.dataPengeluaranPersembahanDiakonia');
            }
            return redirect()->route('pengurusharian.datapersembahanUmum');
        }
    }
    ...
    public function ubahpengeluarankas($id_user, $id)
    {
        ...
        $kas  = Kas::all();
        return view('Pengurusharian.editdataPengeluaranKas', compact('pengeluaran', 'kas'));
    }
    ...
    public function updatepengeluarankas(Request $request, $id_user, $id)
    {
        ...
        if (!$keuangan) {
            return redirect()->route('pengurusharian.pengeluarankas')->with('error', 'Data tidak ditemukan');
        }
        ...
        return redirect()->route('pengurusharian.pengeluarankas')->with('success', 'Data berhasil diperbarui');
    }
    ...
    public function ubahpemasukanpersembahanibadah($id_user, $id)
    {
        ...
        return view('Pengurusharian.formubahpersembahanibadah', [
            'keuangan' => $keuangan,
            'kasList' => $kasList,
            'kases' => $kases
        ]);
    ...
    public function updatepemasukanpersembahanibadah(Request $request, $id_user, $id)
    {
        ...
        if (!$hasChanges) {
            return redirect()->route('pengurusharian.dataPemasukanPersembahanIbadah')->with('warning', 'Tidak ada perubahan pada data Pemasukan Persembahan Ibadah!');
        }

        return redirect()->route('pengurusharian.dataPemasukanPersembahanIbadah')->with('success', 'Data berhasil diperbarui');
    }
    public function formkas($id_user)
    {
        return view('Pengurusharian.formtambahkas');
    }
    ...
    public function formkasprocess(Request $request, $id_user)
    {
        ...
        try {
            ...
            return redirect()->route('pengurusharian.dataPembagianPersembahanIbadah')->with('success', 'Data kas berhasil ditambahkan');
        } catch (\Exception $e) {
    ...
    public function ubahpembagianpersentase($id_user)
    {
        ...
        return view('Pengurusharian.formubahPersentase', [
            'kasList' => $kasList,
        ]);
    }
    ...
    public function updatepembagianpersentase(Request $request, $id_user)
    {
        try {
            ...
            return redirect()->route('pengurusharian.dataPembagianPersembahanIbadah')->with('success', 'Pembagian persentase berhasil diperbarui');
        } catch (\Exception $e) {
    ...
    public function ubahpengeluaranpersembahanibadah($id_user, $id)
    {
        ...
        return view('Pengurusharian.formubahpersembahanibadah', ['keuangan' => $keuangan]);
    }
    ...
    public function updatepengeluaranpersembahanibadah(Request $request, $id_user, $id)
    {
        ...
        $keuangan = Keuangan::find($id);
        if (!$keuangan) {
            return redirect()->route('pengurusharian.dataPengeluaranPersembahanIbadah')->with('error', 'Data tidak ditemukan');
        }
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.dataPengeluaranPersembahanIbadah')->with('warning', 'Tidak ada perubahan pada data Pengeluaran Persembahan Ibadah!');
        }
        ...
        return redirect()->route('pengurusharian.dataPengeluaranPersembahanIbadah')->with('success', 'Data berhasil diperbarui');
    }
    ...
    {
        $keuangan = DB::table('keuangan')->where('id', $id)->get();
        return view('Pengurusharian.formubahpersembahandiakoni', ['keuangan' => $keuangan]);
    }
    ...
    public function updatepemasukanpersembahandiakonia(Request $request, $id_user, $id)
    {
        ...
        $keuangan = Keuangan::find($id);
        if (!$keuangan) {
            return redirect()->route('pengurusharian.dataPemasukanPersembahanDiakonia')->with('error', 'Data tidak ditemukan');
        }
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.dataPemasukanPersembahanDiakonia')->with('warning', 'Tidak ada perubahan pada data Pemasukan Persembahan Diakonia!');
        }
        ...
        return redirect()->route('pengurusharian.dataPemasukanPersembahanDiakonia')->with('success', 'Data berhasil diperbarui');
    }
    ...
    public function updatepengeluaranpersembahandiakonia(Request $request, $id_user, $id)
    {
        ...
        $keuangan = Keuangan::find($id);
        if (!$keuangan) {
            return redirect()->route('pengurusharian.dataPengeluaranPersembahanDiakonia')->with('error', 'Data tidak ditemukan');
        }
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.dataPengeluaranPersembahanDiakonia')->with('warning', 'Tidak ada perubahan pada data Pengeluaran Persembahan Diakonia!');
        }
        ...
        return redirect()->route('pengurusharian.dataPengeluaranPersembahanDiakonia')->with('success', 'Data berhasil diperbarui');
    }
    ...
    public function ubahpemasukanpersembahankhusus($id_user, $id)
    {
        ...
        return view('Pengurusharian.formubahpersembahankhusus', compact('keluarga', 'keuangan'));
    }
    ...
    public function updatepemasukanpersembahankhusus(Request $request, $id_user, $id)
    {
        ...
        if (!$keuangan) {
            return redirect()->route('pengurusharian.datapemasukanpersembahanKhusus')->with('error', 'Data tidak ditemukan');
        }
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.datapemasukanpersembahanKhusus')->with('warning', 'Tidak ada perubahan pada data Pemasukan Persembahan Khusus!');
        }
        ...
        return redirect()->route('pengurusharian.datapemasukanpersembahanKhusus')->with('success', 'Data berhasil diperbarui');
    }
    ...
    public function updatepengeluaranpersembahankhusus(Request $request, $id_user, $id)
    {
        ...
        $keuangan = PersembahanKhusus::find($id);
        if (!$keuangan) {
            return redirect()->route('pengurusharian.datapengeluaranpersembahanKhusus')->with('error', 'Data tidak ditemukan');
        }
        ...
        // Bandingkan data baru dengan data yang ada
        if ($newData == $currentData) {
            return redirect()->route('pengurusharian.datapengeluaranpersembahanKhusus')->with('warning', 'Tidak ada perubahan pada data Pengeluaran Persembahan Khusus!');
        }
        ...
        return redirect()->route('pengurusharian.datapengeluaranpersembahanKhusus')->with('success', 'Data berhasil diperbarui');
    }
    ...
    public function ubahpembagianpersembahanibadah($id_user, $id)
    {
        ...
        return view('Pengurusharian.formubahPembagianpersembahanibadah', compact('keuangan'));
    }
    ...
    public function updatepembagianpersembahanibadah(Request $request, $id_user, $id)
    {
        ...
        try {
            ...
            return redirect()->route('pengurusharian.dataPembagianPersembahanIbadah')->with('success', 'Data kas berhasil diupdate');
        } catch (\Exception $e) {
            // Jika terjadi kesalahan, tangani di sini
    ...
    //         'keterangan' => 'required|string|max:255',
    //         'tanggal' => 'required|date',
    //         'kasPelayananGereja' => 'nullable|numeric|min:0',
    //         'kasKantorSinode' => 'nullable|numeric|min:0',
    //         'kasSaranaPrasarana' => 'nullable|numeric|min:0',
    //         'kasPembangunan' => 'nullable|numeric|min:0',
    //         'total_nominal_kas' => 'nullable|numeric|min:0',
    //     ]);

    //     // Temukan data keuangan berdasarkan ID
    //     $keuangan = Keuangan::with('kasKeuangan')->findOrFail($id);

    //     // Data keuangan baru yang akan diupdate
    //     $dataBaru = [
    //         'keterangan' => $request->keterangan,
    //         'tanggal' => $request->tanggal,
    //         'total_nominal_kas' => $request->total_nominal_kas,
    //         'kasPelayananGereja' => $request->kasPelayananGereja,
    //         'kasKantorSinode' => $request->kasKantorSinode,
    //         'kasSaranaPrasarana' => $request->kasSaranaPrasarana,
    //         'kasPembangunan' => $request->kasPembangunan,
    //     ];

    //     // Data keuangan lama yang ada di database
    //     $dataLama = [
    //         'keterangan' => $keuangan->keterangan,
    //         'tanggal' => $keuangan->tanggal,
    //         'total_nominal_kas' => $keuangan->nominal,
    //         'kasPelayananGereja' => optional($keuangan->kasKeuangan->where('id_kas', 1)->first())->nominal,
    //         'kasKantorSinode' => optional($keuangan->kasKeuangan->where('id_kas', 2)->first())->nominal,
    //         'kasSaranaPrasarana' => optional($keuangan->kasKeuangan->where('id_kas', 3)->first())->nominal,
    //         'kasPembangunan' => optional($keuangan->kasKeuangan->where('id_kas', 4)->first())->nominal,
    //     ];

    //     // Periksa apakah tidak ada perubahan pada data atau nominal adalah null
    //     if ($dataBaru == $dataLama || is_null($request->total_nominal_kas)) {
    //         return redirect()->route('pengurusharian.dataPembagianPersembahanIbadah')->with('warning', 'Tidak ada perubahan pada data');
    //     }

    //     // Update data keuangan utama
    //     $keuangan->update([
    //         'keterangan' => $request->keterangan,
    //         'tanggal' => $request->tanggal,
    //         'nominal' => $request->total_nominal_kas,
    //     ]);

    //     // Hapus data kas yang terkait
    //     KasKeuangan::where('id_keuangan', $id)->delete();

    //     // Simpan data kas baru
    //     if ($request->kasPelayananGereja) {
    //         KasKeuangan::create([
    //             'id_kas' => 1,
    //             'id_keuangan' => $keuangan->id,
    //             'nominal' => $request->kasPelayananGereja,
    //         ]);
    //     }
    //     if ($request->kasKantorSinode) {
    //         KasKeuangan::create([
    //             'id_kas' => 2,
    //             'id_keuangan' => $keuangan->id,
    //             'nominal' => $request->kasKantorSinode,
    //         ]);
    //     }
    //     if ($request->kasSaranaPrasarana) {
    //         KasKeuangan::create([
    //             'id_kas' => 3,
    //             'id_keuangan' => $keuangan->id,
    //             'nominal' => $request->kasSaranaPrasarana,
    //         ]);
    //     }
    //     if ($request->kasPembangunan) {
    //         KasKeuangan::create([
    //             'id_kas' => 4,
    //             'id_keuangan' => $keuangan->id,
    //             'nominal' => $request->kasPembangunan,
    //         ]);
    //     }
    ...
    //     return redirect()->route('pengurusharian.dataPembagianPersembahanIbadah')->with('success', 'Data berhasil diubah');
    // }
    ...
    public function profile($nik)
    {
        ...
        $file_surat_baptis = $baptis ? explode("#", $baptis['file_surat_baptis']) : [];
        $file_surat_sidi = $sidi ? explode("#", $sidi['file_surat_sidi']) : [];
        return view('Pengurusharian.profile', ['jemaat' => $jemaat, 'lampiran' => $lampiran, 'baptis' => $baptis, 'sidi' => $sidi, 'file_surat_baptis' => $file_surat_baptis, 'file_surat_sidi' => $file_surat_sidi, 'sektors' => $sektors]);
    }

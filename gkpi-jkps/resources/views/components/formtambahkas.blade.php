...
<div class="col-12 p-3 bg-white shadow rounded">
    ...
    <form class="mt-3" action="@if ($navbars == StaticVariable::$navbarPengurusHarian) {{ route('pengurusharian.formproseskas',['id_user' => StaticVariable::$user->nik]) }} @elseif($navbars == StaticVariable::$navbarPendeta) {{ route('pendeta.formproseskas',['id_user' => StaticVariable::$user->nik]) }} @endif" method="post">
        @csrf
        <div class="row">
            <div class="form-group col-6 mt-3">
                <label for="nama_kas">Nama Kas</label>
                <input type="text" class="form-control" name="nama_kas" id="nama_kas" required>
            </div>
        </div>

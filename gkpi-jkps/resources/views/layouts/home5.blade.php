...
// Ambil nilai nik dari StaticVariable::$user
$userNik = StaticVariable::$user->nik;

// Menggunakan Eloquent untuk mengambil profil berdasarkan nik
$jemaat = Jemaat::where('nik', $userNik)->first();

// Periksa apakah data jemaat ditemukan
if ($jemaat) {
    // Gunakan profil untuk menampilkan gambar
    echo '<img class="rounded-circle" src="' . asset($jemaat->profile) . '" alt="">'. $jemaat->name;
} else {
    echo "Tidak ada data jemaat yang ditemukan untuk nik = '$userNik'";
}
?>
                        {{-- {{ StaticVariable::$user->name }} --}}
                    </button>
                    ...
                    {{-- <form action="@if ($navbars == StaticVariable::$navbarPengurusHarian) {{ route('pengurusharian.updateFotoProfil', ['nik' => $jemaat['nik']]) }} @elseif($navbars == StaticVariable::$navbarPendeta) {{ route('pendeta.updateFotoProfil', ['nik' => $jemaat['nik']]) }} @elseif($navbars == StaticVariable::$navbarPenatua) {{ route('penatua.updateFotoProfil', ['nik' => $jemaat['nik']]) }} @elseif($navbars == StaticVariable::$navbarJemaat) {{ route('jemaat.updateFotoProfil', ['nik' => $jemaat['nik']]) }}@endif" method="post" enctype="multipart/form-data"> --}}
                        ...
                        <a class="dropdown-item" href="/pengurusharian/{{ StaticVariable::$user->nik }}/profile">
